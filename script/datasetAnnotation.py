...
for _, row in df.iterrows():
    ...
    annotations[fname] = {
        ...
        "head_pose": None,
        "face_grid": None,
        "gaze_vector": ([float(row['v3d_x']), float(row['v3d_y']), float(row['v3d_z'])]
                        if not pd.isna(row['v3d_x']) else None),
        "screen_gaze": ([float(row['x2d']),  float(row['y2d'])]
                        if not pd.isna(row['x2d']) else None),
        "gaze_angles": ([float(row['yaw']),  float(row['pitch'])]
